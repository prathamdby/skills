...
const quickstartSection = sectionBetween(readme, '3. Invoke them in your agent:', '## Why These Skills Exist');
...
for (const dir of skillDirs) {
  ...
  if (hasReference) requireStepZero(text, skillPath);
}
...
function requireKebabName(name, dir, filePath) {
  ...
  if (name !== dir) fail(filePath, `name "${name}" does not match directory "${dir}"`);
  if (name.length > 64) fail(filePath, 'name is longer than 64 characters');
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(name)) fail(filePath, 'name must be kebab-case');
}

function requireDescription(description, filePath) {
  if (description.length > 1024) fail(filePath, 'description is longer than 1024 characters');
  if (!/\bUse when\b/.test(description)) fail(filePath, 'description must include "Use when" triggers');
}

function requireLineLimit(text, filePath) {
  const body = text.endsWith('\n') ? text.slice(0, -1) : text;
  const lineCount = body.length === 0 ? 0 : body.split(/\r?\n/).length;
  if (lineCount > 100) fail(filePath, `SKILL.md has ${lineCount} lines, expected 100 or fewer`);
}

function requireReadmeCoverage(dir, filePath) {
  if (!quickstartSection.includes(`/${dir}`)) fail(filePath, `README quickstart is missing /${dir}`);
  if (!referenceSection.includes(`./skills/${dir}/SKILL.md`)) {
    fail(filePath, `README reference table is missing ./skills/${dir}/SKILL.md`);
  }
}
...
function requireExistingMarkdownReferences(text, filePath) {
  ...
  for (const target of references) {
    ...
    if (!existsSync(resolved)) fail(filePath, `referenced file does not exist: ${target}`);
  }
}

function requireStepZero(text, filePath) {
  const stepZero = text.indexOf('## Step 0: Read REFERENCE.md (mandatory)');
  if (stepZero === -1) {
    fail(filePath, 'REFERENCE.md exists but mandatory Step 0 is missing');
    return;
  }

  const firstWorkStep = text.search(/\n## (Step 1|Workflow A)/);
  if (firstWorkStep !== -1 && stepZero > firstWorkStep) {
    fail(filePath, 'Step 0 must appear before Step 1 or Workflow A');
  }

  const gateText = text.slice(stepZero, firstWorkStep === -1 ? undefined : firstWorkStep);
  if (!gateText.includes('Do not proceed')) fail(filePath, 'Step 0 is missing the Do not proceed warning');
  if (!gateText.includes('./REFERENCE.md')) fail(filePath, 'Step 0 must instruct agents to read ./REFERENCE.md');
  if (!gateText.includes('Skipping this step causes')) {
    fail(filePath, 'Step 0 must name the failure caused by skipping it');
  }

  const constraints = text.match(/\n## Constraints[\s\S]*$/);
  if (constraints && !constraints[0].includes('Never skip Step 0')) {
    fail(filePath, 'Constraints must include Never skip Step 0');
  }
}
