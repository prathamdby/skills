...
| An M or L PR body leaves a shape change as prose when a sketch would make the delta clear. | [`make-pr`](./skills/make-pr/SKILL.md) | Picks the smallest proved view from Body visuals and places it next to the theme it supports. |
...
| [`make-pr`](./skills/make-pr/SKILL.md) | Publish a branch and create or update its PR with a diff-scaled body and proved visuals. | `--target <branch>` default `main`, `--ticket <id>`, `--conventional` |
