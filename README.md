...
- `/upfront-design` gates product review, architecture, program design, and slices on user approval before code, then checks each landed slice against the design.
...
| The agent starts coding from a one-line request; the PR needs rework and review drags.                                      | [`upfront-design`](./skills/upfront-design/SKILL.md) | Triages size, writes each approved design phase to a file outside the repo, records only hard-to-reverse decisions as ADRs, and checks landed slices against the design. |
...
| [`upfront-design`](./skills/upfront-design/SKILL.md) | Agree product review, architecture, program design, and vertical slices before code; check landed slices.   | positional request, `--resume <path>`, `--check <path>`, `--output <path>` default anchor path                                                                                                       |
