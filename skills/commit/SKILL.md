...
2. Hooks: hooks off means `-n`; hooks on never bypasses hooks. A failed
...
Record
`scope | style | hooks=off|on | trailers=deny|allow | selected diff hash | paths | message | command | commit | terminal`.
...
Omit `-n` only when hooks are on. Pass subject and body as separate argv values
...
commit, and when trailers are allowed but `%B` has an unrequested banned key
or freeform harness line, open a Python or Node.js REPL and read `HEAD` `%B`.
Strip only those unrequested lines. Keep requested trailers when allowed.
