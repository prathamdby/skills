...
Clone the external repository into a sandbox next to this skill file and answer from that local copy. Keep a small manifest there that maps each repository name to its URL and local path. Pull to update a clone that already exists. Never put sandbox data inside the user's working directory.
