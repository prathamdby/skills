...
- Body: measure the locked diff, pick the tier, and draft per Body scale and
  Body style in `./REFERENCE.md`. For every tier, including S, add Body
  visuals for every theme with a proved shape. Classify that shape against
  Pick the views in `./REFERENCE.md` and place each view next to its theme.
  Group related hunks, never commits. Include no test, rollout, motive,
  ticket claim the diff cannot prove, or harness footer.
...
Body visuals, and clean-room trace pass.
