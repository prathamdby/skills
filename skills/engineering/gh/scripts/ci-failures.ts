...
const MARKERS =
  /##\[error\]|\berror\b|\bfail(?:ed|ure)?\b|exception|traceback|panic|fatal/i;
const UNZIP_PY = `
import sys, zipfile
...
function snippet(log: string, before = 40, after = 5, cap = 100): string {
  ...
  const ringSnap = (): string[] =>
    ring.length < before
      ? ring.slice()
      : ring.slice(head).concat(ring.slice(0, head));

  let bestError: string[] | null = null;
  let bestMark: string[] | null = null;
  let collect: { lines: string[]; remain: number; error: boolean } | null =
    null;
...
function leadSnippet(annotations: Ann[], log: string): string {
  ...
  const rows = annotations.map(
    (a) => `${a.path}:${a.start_line} [${a.annotation_level}] ${a.message}`,
  );
  const extra =
    rows.length > cap ? [`… ${rows.length - cap} more annotations`] : [];
...
async function downloadLog(
  repo: string,
  job: { databaseId: number; name: string },
  logDir: string,
): Promise<JobLog & { text?: string }> {
  try {
    ...
    if (msg.includes("404"))
      return { error: "log not available yet (job still running?)" };
...
async function analyzeRun(
  ...
  const rv = await ghJson<RunView>([
    ...
    repo,
    "--json",
    "jobs,workflowName,conclusion,url",
  ]);
  const failingJobs = rv.jobs.filter((j) =>
    FAILING.has((j.conclusion ?? "").toLowerCase()),
  );
    ...
    failingJobs.map(async (j) => {
      ...
      const log: JobLog = downloaded.error
        ...
        : {
            ...
            snippet: leadSnippet(anns, downloaded.text ?? ""),
          };
      return {
        name: j.name,
        ...
        url: j.url,
        failedSteps: (j.steps ?? [])
          .filter((s) => FAILING.has((s.conclusion ?? "").toLowerCase()))
          .map((s) => s.name),
        log,
      };
    }),
...
async function failingFromSha(
  repo: string,
  sha: string,
): Promise<{ runs: Map<string, string[]>; external: Check[] }> {
  ...
  for (let page = 1; page <= 10; page++) {
    ...
    for (const c of rows) {
      ...
      const bucket = FAILING.has(conclusion)
        ? "fail"
        : conclusion === "success"
          ? "pass"
          : "pending";
...
run(async () => {
  ...
  if (v.list) {
    ...
    if (!Number.isInteger(limit) || limit <= 0)
      throw new Error(`-L must be a positive number, got: ${v.limit}`);
    const args = [
      "run",
      "list",
      "-R",
    ...
    if (v.json)
      return void console.log(JSON.stringify({ repo, runs: rows }, null, 2));
    log(
      `${repo}: last ${rows.length} runs${v.workflow ? ` · workflow ${v.workflow}` : ""}`,
    );
    for (const r of rows) {
      const mark =
        r.conclusion === "success"
          ? "✓"
          : FAILING.has(r.conclusion)
            ? "✗"
            : "○";
      const concl = r.conclusion || r.status;
      const when = r.createdAt.slice(0, 16).replace("T", " ");
      const title =
        r.displayTitle && r.displayTitle !== r.workflowName
          ? ` · ${truncate(r.displayTitle, 48)}`
          : "";
      log(
        `${mark} ${r.databaseId}  ${when}  ${concl.padEnd(11)} ${r.event.padEnd(17)} ${r.workflowName}${title}`,
      );
    }
    if (rows.length > 0)
      log(
  ...
  if (runId) {
    ...
    if (v.pr && !/^\d+$/.test(v.pr))
      throw new Error(`--pr must be a number, got: ${v.pr}`);
    if (!v.pr && v.repo)
      throw new Error("with -R, also pass --pr N or a run id");
    prNum = v.pr
      ? Number(v.pr)
      : (await ghJson<{ number: number }>(["pr", "view", "--json", "number"]))
          .number;
    if (v.sha) {
    ...
    if (runs.size === 0 && external.length === 0) {
      if (v.json)
        return void console.log(
          JSON.stringify({ repo, pr: prNum, runs: [], external: [] }, null, 2),
        );
  ...
  const logDir =
    runs.size > 0
      ? mkdtempSync(join(process.env.TMPDIR || tmpdir(), "gh-ci-"))
      : "";
  ...
  if (v.json) {
    return void console.log(
      JSON.stringify({ repo, pr: prNum, runs: results, external }, null, 2),
    );
  }

  const prLabel = prNum ? ` PR #${prNum}` : "";
  log(
    `${repo}${prLabel}: ${results.length} run${results.length === 1 ? "" : "s"} analyzed\n`,
  );
  for (const r of results) {
    ...
    if (r.jobs.length === 0 && !FAILING.has(r.conclusion)) {
      log(
        `○ ${r.workflow} · run ${r.runId} concluded ${r.conclusion || "in progress"}, nothing to report\n`,
      );
      continue;
    }
    ...
    if (r.jobs.length === 0)
      log(
        "  no failing jobs; failure is at the workflow level (startup/config?)",
      );
    for (const j of r.jobs) {
      const steps = j.failedSteps.length
        ? `, failed step: ${j.failedSteps.join(", ")}`
        : "";
  ...
  for (const c of external)
    log(
      `✗ ${c.name} is an external check (not GitHub Actions): ${c.link || "(no link)"}`,
    );
});
